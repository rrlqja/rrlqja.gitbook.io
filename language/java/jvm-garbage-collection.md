...
g1 gc는 young 영역과 old 영역을 물리적으로 구분 짓는 것이 아닌 지역(region)이라는 개념을 통해 영역과 역할을 동적으로 할당한다. 다른 gc는 eden, survivor, old 영역에 순차적으로 저장됐지만 g1 gc는 효율적인 영역을 판단해객체를 할당 시킨다.
